...
const MenuItemShowPopup = (props) => {
    
    return (
        <Observer>
            {()=>(
                <Modal onOverlayClick={()=>{props.menuStore.showMenuItemShowPopup = false}}>
            <div className=" white-bg border-radius-sm modal-wrapper modal-lg">
                <div className="flex space-between padding-bottom-sm padding-lg border-bottom">
                    ...
                    <span style={{fontSize:22}} className="pointer" onClick={()=>{props.menuStore.showMenuItemShowPopup = false}}>X</span>
                </div>
                <div className="modal-body padding-lg">
                    ...
                    <div className="margin-top-lg flex align-center">
                        <Button type="button" disabled={props.menuStore.menuAddLoading || (!props.menuStore.menuaddItem.name)}  onClick= { ()=>{props.menuStore.createMenuitem(true)}} text="Add" type={AppConstant.POSITIVE} />
                    </div>
...
export default MenuItemShowPopup;