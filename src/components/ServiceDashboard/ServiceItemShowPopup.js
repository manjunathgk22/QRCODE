...
const ServiceItemShowPopup = (props) => {
    
    return (
        <Observer>
            {()=>(
                <Modal onOverlayClick={()=>{props.serviceStore.showServicesItemShowPopup = false}}>
            <div className=" white-bg border-radius-sm modal-wrapper modal-lg">
                <div className="flex space-between padding-bottom-sm padding-lg border-bottom">
                    ...
                    <span style={{fontSize:22}} className="pointer" onClick={()=>{props.serviceStore.showServicesItemShowPopup = false}}>X</span>
                </div>
                <div className="modal-body padding-lg">
                    ...
                    <div className="margin-top-lg flex align-center">
                        <Button type="button" disabled={props.serviceStore.serviceAddLoading || (!props.serviceStore.serviceaddItem.name)}  onClick= { ()=>{props.serviceStore.createServiceitem(true)}} text="Add" type={AppConstant.POSITIVE} />
                    </div>
...
export default ServiceItemShowPopup;