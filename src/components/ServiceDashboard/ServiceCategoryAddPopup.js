...
const ServiceCategoryAddPopup = (props) => {
    return (
        <Observer>
            {()=>(
                <Modal onOverlayClick={()=>{props.serviceStore.showServiceCategoryAddPopup = false}}>
            <div className=" white-bg border-radius-sm modal-wrapper">
                <div className="flex space-between padding-bottom-sm padding-lg border-bottom">
                    ...
                    <span style={{fontSize:22}} className="pointer" onClick={()=>{props.serviceStore.showServiceCategoryAddPopup = false}}>X</span>
                </div>
                <div className="modal-body padding-lg">
                    ...
                    <div className="margin-top-lg flex align-center">
                        <Button type="button" disabled={props.serviceStore.serviceAddLoading || (!props.serviceStore.serviceaddCategory.name)}  onClick= { ()=>{props.serviceStore.createServiceCategory()}} text="Add" type={AppConstant.POSITIVE} />
                    </div>
                </div>
            </div>
        </Modal>
...
export default ServiceCategoryAddPopup;